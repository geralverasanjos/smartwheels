  ...
  .onCreate(async (snap, context) => {
    ...
    for (const driverDoc of nearbyDriverDocs) {
      ...
      if (driverProfile && driverProfile.status === 'online' && driverProfile.activeVehicleId) {
        ...
        // Match service type with vehicle capabilities
        const serviceCategory = serviceType.startsWith('delivery') ? 'deliveries' : 'passengers';
        
        if (vehicle && vehicle.status === 'active' && vehicle.allowedServices.includes(serviceCategory)) {
