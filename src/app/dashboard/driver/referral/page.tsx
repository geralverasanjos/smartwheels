...
export default function ReferralPage() {
    ...
    const handleShare = async () => {
        const shareData = {
            ...
            text: t('referral_share_text_driver', { code: referralData.referralCode }),
            url: referralLink,
        };
        try {
    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="panel-header">
                <h1 className="font-headline title-glow">{t('refer_page_title_driver')}</h1>
                <p>{t('refer_page_subtitle_driver')}</p>
            </div>

            <Card className="text-center">
                <CardHeader>
                    <Gift className="mx-auto h-12 w-12 text-primary" />
                    <CardTitle className="text-2xl mt-2">{t('refer_card_title_driver')}</CardTitle>
                    <CardDescription>{t('refer_card_desc_driver', { friend_bonus: '€10', your_bonus: '€10' })}</CardDescription>
                </CardHeader>
