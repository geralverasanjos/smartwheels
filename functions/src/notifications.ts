...
/**
 * Creates a notification for the passenger when a driver accepts their ride request.
 */
  ...
  .onUpdate(async (change, context) => {
    ...
    if (after.status === 'accepted' && before.status !== 'accepted') {
      ...
      const notificationPayload = {
        ...
        descriptionParams: { name: driverName }, // Dynamic data for translation
        timestamp: admin.firestore.FieldValue.serverTimestamp(),
        isRead: false,
        link: `/dashboard/passenger/transport` // Link to the trip tracking page
      };

      try {
        await db.collection('notifications').add(notificationPayload);
        console.log(`Notification created for passenger ${passengerId} for ride ${context.params.requestId}`);
      } catch (error) {
